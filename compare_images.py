...
def compare_slices(images, titles, slice_idx=None, coeff_idx=0):
    """比较不同图像的同一个切片的特定系数"""
    ...
    for idx, (img, title) in enumerate(zip(images, titles)):
        ...
        axes[idx].set_title(f"{title}\nSlice {slice_idx}, Coeff {coeff_idx}")
        plt.colorbar(im, ax=axes[idx])
    
    plt.tight_layout()
    plt.savefig(f'slice_comparison_coeff_{coeff_idx}.png')
    plt.close()
...
def main():
    ...
    for coeff_idx in [0, 22, 44]:  # 比较第一个、中间和最后一个系数
        compare_slices(images, [f"{r}% Training" for r in ratios], coeff_idx=coeff_idx)
    
    print("\n图像对比已保存为 slice_comparison_coeff_*.png")
